...
async function runSample(sample, idx, outputSize) {
  ...
  if (!process.env.GEMINI_API_KEY) {
    console.log('  ⚠️ GEMINI_API_KEY 미설정 → claude-extractor가 시드 기반 폴백 사용 ' +
                '(텍스트 의미는 분석되지 않고 diversitySeed로 결정론적 값 생성)');
  }
  ...
  // emotion-engine의 ECardResult를 reply-engine이 기대하는
  // ExtractionResult 형식(emotionScores/dominantEmotion/spotIndex/
  // contextAnalysis/primaryEmotion/keywords)으로 재구성한다.
  // (server/routes/impression.js가 emotionResult 자체를 그대로
  //  classify()에 전달하는 것과 동일한 방식)
  const extractionLike = {
    ...
    contextAnalysis: {
      ...
      companionContext: { detected: emotionResult.context?.companionContext ?? null, confidence: 0 },
    },
  };
